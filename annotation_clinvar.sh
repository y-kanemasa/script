...
for file in ../database/clinvar_*.hg19.recode2.vcf.gz; do
  ...
  # ファイルが存在するか確認
  if ! curl --head --silent --fail "https://ftp.ncbi.nlm.nih.gov/pub/clinvar/vcf_GRCh37/clinvar_${date}.vcf.gz" > /dev/null; then
...
# ANN, NMD, faf95_g2ex, faf95_g2ge, faf95_g3ge_popmax, SwappedAlleles, ReverseComplementedAlleles
paste -d , <(awk -F"," -v "OFS=," '{print $2":"$3"_"$5"_"$6,$4}' short-variant_tr.change2.combined6.snpeff.sort.annotate.csv) <(cut -d , -f 9- short-variant_tr.change2.combined6.snpeff.sort.annotate.csv) | awk -F, '{for (i=1; i<=NF; i++) if (i!=4 && i!=6 && i!=19 && i!=24 && i!=27 && i!=30 && i!=31) printf "%s%s", $i, (i==NF?"\n":",")}' > short-variant_tr.change2.combined6.snpeff.sort.annotate2.csv
...
# 62を変更
diff <(sed '1d' short-variant_tr.change2.combined6.snpeff.sort.annotate9.csv | cut -d , -f 1) <(sed '1d' short-variant_tr.change2.combined6.snpeff.sort.annotate9.csv | cut -d , -f 62)
